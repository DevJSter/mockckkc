...
export function useLatestBlocks() {
  ...
  const SUBGRAPH_HASH = 'QmTMznGq4D8jqe3W6Vx2P8gceMQtgFet9Z4p6vfRCR2ukh' // your actual deployment hash here
  ...
  useEffect(() => {
    ...
    const fetchSubgraphStatus = async () => {
      try {
        const { data } = await healthClient.query({
          query: SUBGRAPH_HEALTH,
          variables: {
            subgraphs: [SUBGRAPH_HASH],
          },
          fetchPolicy: 'no-cache',
        })

        const status = data?.indexingStatuses?.[0]
        const chains = status?.chains

        if (!status || !chains || chains.length === 0) return

        const latest = Number(chains[0]?.latestBlock?.number)
        const head = Number(chains[0]?.chainHeadBlock?.number)

        if (isMounted) {
          if (!isNaN(latest)) updateLatestBlock(latest)
          if (!isNaN(head)) updateHeadBlock(head)
        }
      } catch (err) {
        console.error('Graph Indexer health fetch failed:', err)
      }
    }
